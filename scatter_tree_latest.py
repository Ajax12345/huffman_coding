...
class BlockTree:
    ...
    def __init__(self, _start = 0, **kwargs):
        ...
        self.row = [[c, BlockTree(depth = kwargs.get('depth', 0)+1, rotation=self.rotation, new_depth = kwargs.get('new_depth', 0)+1) if kwargs.get('depth', 0) < 5 else None] for c in results1[BlockTree.current_depth%len(results1)]]
    ...
    def _full_node_count(self):
        return sum(1+getattr(b, '_full_node_count', lambda :0)() for _, b in self.row)
