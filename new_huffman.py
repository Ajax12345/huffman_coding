...
class Decompress:
        ...
        while True:
            ...
            if not possibilities:
                ...
                if _hashed[possibilities[0]] == 'F':
                    ...
                    if content[len(possibilities[0]):].startswith('11001011100101'):

                        _temp_possibilites = [i for i in _hashed if content[len(possibilities[0])+len('11001011100101'):].startswith(i)]
                        print('_temp_possibilites', _temp_possibilites)
                        if not _temp_possibilites or not _hashed[_temp_possibilites[0]].isdigit():
                            content = content[len(possibilities[0])+len('11001011100101'):]
                            break


                header.append(_hashed[possibilities[0]])
                content = content[len(possibilities[0]):]
        return content, Decompress._parse_new_Header(header)
    ...
    @staticmethod
    def _parse_new_Header(header:list, as_dict = True):
        grouped_data = iter([a, list(b)] for a, b in itertools.groupby(header, key=lambda x:x == 'F'))
        final_data = []
        while True:
            current = next(grouped_data, None)
            if not current:
                break
            flag, _result = current
            if not flag:
                final_data.append([_result[0], int(''.join(_result[1:]))])
            else:
                if len(_result) == 3:
                    _, _v = next(grouped_data)
                    final_data.append(['F', int(''.join(_v))])
        return dict(final_data) if as_dict else final_data
...
class Compress:
    ...
    def __enter__(self):
        ...
        _converted_content = '{}1100101{}'.format(''.join(_hashed_alphabet[a]+''.join(_hashed_alphabet[c] for c in str(b))+'11001011100101' for a, b in _frequencies), ''.join(full_hashing[i] for i in self._file_data))
...
if __name__ == '__main__':
    ...
    @time_compression
    def decompress():
        with Decompress('452018422819simplerhobbit.bin') as c:
            pass

        print(('-'*10)+'compression finished'+('-'*10))
        print(size_comparison('simplerhobbit.txt', '452018422819simplerhobbit.bin'))

    def size_comparison(a, b):
        _a, _b = os.path.getsize(a), os.path.getsize(b)
        return '{}: {}, {}(compressed): {}. Total difference: {} bytes for {}% reduction'.format(a, _a, b, _b, _a - _b, (1-((_b)/float(_a)))*100)
